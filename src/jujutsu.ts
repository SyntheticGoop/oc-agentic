    ...
    return {
      async new(options?: {
        ...
        return spawnjj(args).then((result) => {
          ...
          if (typeof change !== "string")
            return Err("Unexpected missing created change id");
        ...
        return spawnjj(["log", "-r", "@", "--no-graph", "-R", dir]).then(
          (result) => {
            ...
            if (typeof change?.groups?.change !== "string")
              return Err("Unable to get change id");
      ...
      history: {
        async linear() {
          ...
          if (currentParts.length < 3) return Err("Unexpected commit format");
          ...
          for (const line of ancestorLines) {
            ...
            if (parts.length < 3) return Err("Unexpected commit format");
