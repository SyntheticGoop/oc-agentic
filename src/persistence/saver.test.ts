...
describe("Format Generation Tests", () => {
  ...
  it("should generate SHORT format for single task plans", async () => {
    ...
    // commits with double colon (::) syntax for single task plans. The
...
`;
    ...
    const workingLoadResult = await loader.loadPlan();

    // Verify SHORT format was created
    const descResult2 = await jj.description.get();
    expect(descResult2.err).toBeUndefined();
    if (descResult2.ok) {
      expect(descResult2.ok).toContain("feat(test)::~"); // Double colon with incomplete marker
...
`;
    ...
    if (historyResult.ok) {
      ...
      // Should contain task commits with single colon
...
describe("New Plan Creation Tests", () => {
  ...
  it("should create SHORT format for single task plans", async () => {
    ...
    expect(descResult.err).toBeUndefined();
    if (descResult.ok) {
      expect(descResult.ok).toContain("feat(test)::~"); // Double colon with incomplete marker
